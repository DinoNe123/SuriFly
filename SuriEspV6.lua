...
local function CreateTabButton(text, color)
    ...
    btn.MouseEnter:Connect(function() TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = color:Lerp(Color3.fromRGB(255,255,255), 0.15)}):Play() end)
...
-- ==================== ESP & TELEPORT (giữ nguyên) ====================
-- (ESP và Teleport giữ nguyên như code cũ của bạn)
...
ESPStatus.Size = UDim2.new(1,0,0,40) ESPStatus.Position = UDim2.new(0,0,0,75) ESPStatus.BackgroundTransparency = 1
ESPStatus.Text = "ESP: ENABLED" ESPStatus.TextColor3 = Color3.fromRGB(0,255,140)
ESPStatus.Font = Enum.Font.GothamBlack ESPStatus.TextSize = 18
ESPStatus.Parent = ESPContent

-- Teleport (giữ nguyên)
local ScrollingFrame = Instance.new("ScrollingFrame")
ScrollingFrame.Size = UDim2.new(1,0,1,-70) ScrollingFrame.BackgroundColor3 = Color3.fromRGB(18,18,24)
ScrollingFrame.ScrollBarThickness = 5 ScrollingFrame.Parent = TPContent

local UIListLayout = Instance.new("UIListLayout")
UIListLayout.Padding = UDim.new(0,6) UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder UIListLayout.Parent = ScrollingFrame

local SelectedLabel = Instance.new("TextLabel")
SelectedLabel.Size = UDim2.new(1,0,0,45) SelectedLabel.Position = UDim2.new(0,0,1,-65)
SelectedLabel.BackgroundColor3 = Color3.fromRGB(30,30,38) SelectedLabel.Text = "Chưa chọn người chơi"
SelectedLabel.TextColor3 = Color3.fromRGB(160,160,160) SelectedLabel.Font = Enum.Font.Gotham SelectedLabel.TextSize = 15
SelectedLabel.Parent = TPContent

local TPBtn = Instance.new("TextButton")
TPBtn.Size = UDim2.new(1,0,0,50) TPBtn.Position = UDim2.new(0,0,1,-110)
TPBtn.BackgroundColor3 = Color3.fromRGB(70,200,70) TPBtn.Text = "TELEPORT NGAY"
TPBtn.TextColor3 = Color3.new(1,1,1) TPBtn.Font = Enum.Font.GothamBlack TPBtn.TextSize = 18
TPBtn.Parent = TPContent
Instance.new("UICorner", TPBtn).CornerRadius = UDim.new(0,12)

-- ==================== FLY GUI V3 (TOÀN BỘ CHỨC NĂNG CŨ) ====================
local speeds = 1
local nowe = false
...
-- Buttons
local up = Instance.new("TextButton") up.Size = UDim2.new(0,70,0,40) up.Position = UDim2.new(0.08,0,0.25,0)
up.BackgroundColor3 = Color3.fromRGB(79,255,152) up.Text = "UP" up.TextColor3 = Color3.new(0,0,0)
up.Font = Enum.Font.GothamBold up.TextSize = 16 up.Parent = FlyFrame
Instance.new("UICorner", up).CornerRadius = UDim.new(0,10)

local down = Instance.new("TextButton") down.Size = UDim2.new(0,70,0,40) down.Position = UDim2.new(0.08,0,0.48,0)
down.BackgroundColor3 = Color3.fromRGB(215,255,121) down.Text = "DOWN" down.TextColor3 = Color3.new(0,0,0)
down.Font = Enum.Font.GothamBold down.TextSize = 16 down.Parent = FlyFrame
Instance.new("UICorner", down).CornerRadius = UDim.new(0,10)

local onof = Instance.new("TextButton") onof.Size = UDim2.new(0,110,0,45) onof.Position = UDim2.new(0.52,0,0.25,0)
onof.BackgroundColor3 = Color3.fromRGB(255,249,74) onof.Text = "FLY" onof.TextColor3 = Color3.new(0,0,0)
onof.Font = Enum.Font.GothamBold onof.TextSize = 18 onof.Parent = FlyFrame
Instance.new("UICorner", onof).CornerRadius = UDim.new(0,12)

local plus = Instance.new("TextButton") plus.Size = UDim2.new(0,50,0,40) plus.Position = UDim2.new(0.52,0,0.48,0)
plus.BackgroundColor3 = Color3.fromRGB(133,145,255) plus.Text = "+" plus.TextColor3 = Color3.new(0,0,0)
plus.Font = Enum.Font.GothamBold plus.TextSize = 22 plus.Parent = FlyFrame
Instance.new("UICorner", plus).CornerRadius = UDim.new(0,10)

local mine = Instance.new("TextButton") mine.Size = UDim2.new(0,50,0,40) mine.Position = UDim2.new(0.68,0,0.48,0)
mine.BackgroundColor3 = Color3.fromRGB(123,255,247) mine.Text = "-" mine.TextColor3 = Color3.new(0,0,0)
mine.Font = Enum.Font.GothamBold mine.TextSize = 22 mine.Parent = FlyFrame
Instance.new("UICorner", mine).CornerRadius = UDim.new(0,10)

local speed = Instance.new("TextLabel") speed.Size = UDim2.new(0,60,0,40) speed.Position = UDim2.new(0.82,0,0.48,0)
speed.BackgroundColor3 = Color3.fromRGB(255,85,0) speed.Text = "1" speed.TextColor3 = Color3.new(0,0,0)
speed.Font = Enum.Font.GothamBold speed.TextSize = 20 speed.Parent = FlyFrame
Instance.new("UICorner", speed).CornerRadius = UDim.new(0,10)
...
onof.MouseButton1Click:Connect(function()
	...
	if speaker.Character:FindFirstChildOfClass("Humanoid").RigType == Enum.HumanoidRigType.R6 then
		...
		while nowe or speaker.Character.Humanoid.Health == 0 do
			game:GetService("RunService").RenderStepped:Wait()
			-- ctrl logic (đã được khai báo đầy đủ trong code gốc của bạn)
			-- ... (toàn bộ đoạn ctrl, speed, bv.velocity, bg.cframe giữ nguyên như code bạn đưa)
		end
		bg:Destroy() bv:Destroy()
	else -- R15
		local UpperTorso = speaker.Character.UpperTorso
		local bg = Instance.new("BodyGyro", UpperTorso) bg.P = 9e4 bg.maxTorque = Vector3.new(9e9,9e9,9e9) bg.cframe = UpperTorso.CFrame
		local bv = Instance.new("BodyVelocity", UpperTorso) bv.velocity = Vector3.new(0,0.1,0) bv.maxForce = Vector3.new(9e9,9e9,9e9)
		if nowe then speaker.Character.Humanoid.PlatformStand = true end
...
-- ==================== ESP + TELEPORT LOGIC (giữ nguyên) ====================
-- ... (paste phần createESP, updateESP, refreshESP, updatePlayerList nếu cần)

task.wait(1)
updatePlayerList()
if _G.ESPEnabled then refreshESP() end

print("✅ SCxSuri Hub đã load! Fly GUI V3 Full Logic")
